...
import sys
from pathlib import Path
import matpl        # Display top 5 results in bottom row
        for i, result in enumerate(top_matches):
            ...
            result_image_path = self.data_root / "roxford5k" / "jpg" / f"{base_name}.jpg"yplot as plt
...
class CompletePipeline:
    ...
    def load_ames_model(self):
        ...
        try:
            ...
            if os.path.exists(self.model_path):
                ...
                self.ames_model = AMES(desc_name='dinov2', local_dim=768)
                checkpoint = torch.load(self.model_path, map_location=self.device)
                self.ames_model.load_state_dict(checkpoint['state'], strict=True)
                print("✅ Loaded local AMES model")
            else:
    ...
    def search_image(self, image_path, top_k=5):
        ...
        # Use matplotlib to display query image and top 5 results
        import matplotlib.pyplot as plt
        import matplotlib.patches as patches
        
        # Create figure with subplots: 1 row for query + 1 row for 5 results
        fig = plt.figure(figsize=(20, 8))
        fig.suptitle('AMES Image Search Results', fontsize=16, fontweight='bold')
        ...
        ax_query = plt.subplot(2, 5, (1, 5))  # Top row, spans all 5 columns
        ax_query.imshow(query_img_rgb)
        ax_query.set_title(f'Query: {Path(image_path).name}', fontsize=14, fontweight='bold', pad=20)
        ax_query.axis('off')
        
        # Display top 5 results in bottom row
        for i, result in enumerate(top_matches):
            if i >= 5:  # Only show top 5
                break
            ...
            # Construct full path to result image (base_name should already include .jpg)
            result_image_path = self.data_root / "roxford5k" / "jpg" / base_name
            ...
            try:
                ...
                if result_img is not None:
                    ...
                    # Bottom row subplots (2,5,6) to (2,5,10)
...
def main():
    """Example usage"""
    # Configuration
    image_path = r"C:\gitRepo\ames\data\roxford5k\jpg\all_souls_000013.jpg"
